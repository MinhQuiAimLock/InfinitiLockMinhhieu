...
document.addEventListener("DOMContentLoaded",function(){
  ...
  function reflect(){
    $$(".toggle-switch").forEach(sw=>{
      const input=sw.querySelector(".toggle-input");
      const item=sw.closest(".function-item");
      if(!input||!item) return;
      
      const isChecked = input.checked;
      item.dataset.state = isChecked ? "on" : "off";
      
      if (isChecked) {
        item.style.borderColor = getComputedStyle(item).getPropertyValue('--primary') || 'rgba(34,197,94,.6)';
      } else {
        item.style.borderColor = "rgba(255,255,255,.06)";
      }
    });
  }
  ...
  function showNotification(message){ toast(message); }
  ...
  // EVENT LISTENERS
  if(configToggle) configToggle.addEventListener("change",()=>{store.set("config-enabled",configToggle.checked); runAction("config",configToggle.checked); toast(configToggle.checked?"AIMLOCK : ON":"AIMLOCK : OFF")});
  if(luxToggle)    luxToggle.addEventListener("change",()=>{store.set("lux-enabled",luxToggle.checked); runAction("lux",luxToggle.checked); toast(luxToggle.checked?"FEATHER AIM : ON":"FEATHER AIM : OFF")});
  if(fixRungToggle) fixRungToggle.addEventListener("change",()=>{store.set("fix-rung-enabled",fixRungToggle.checked); runAction("fixRung",fixRungToggle.checked); toast(fixRungToggle.checked?"HEADSHOT FIX : ON":"HEADSHOT FIX : OFF")});
  if(nheTamToggle) nheTamToggle.addEventListener("change",()=>{store.set("nhe-tam-enabled",nheTamToggle.checked); runAction("nheTam",nheTamToggle.checked); setCenterReticleHidden(nheTamToggle.checked); toast(nheTamToggle.checked?"SHAKE FIX : ON":"SHAKE FIX : OFF")});
  if(lockDauToggle) lockDauToggle.addEventListener("change",()=>{store.set("lock-dau-enabled",lockDauToggle.checked); runAction("lockDau",lockDauToggle.checked); toast(lockDauToggle.checked?"DRIFT FIX : ON":"DRIFT FIX : OFF")});
  if(cnMasterToggle) cnMasterToggle.addEventListener("change",()=>{store.set("cn-master-enabled",cnMasterToggle.checked); toast(cnMasterToggle.checked?" SENSIVITY BOOSTER : ON":"SENSIVITY BOOSTER : OFF")});
  if(antiRungProToggle) antiRungProToggle.addEventListener("change",()=>{store.set("anti-rung-pro-enabled",antiRungProToggle.checked); toast(antiRungProToggle.checked?"SCREEN BOOSTER : ON":"SCREEN BOOSTER : OFF")});
  if(aimAssistPlusToggle) aimAssistPlusToggle.addEventListener("change",()=>{store.set("aim-assist-plus-enabled",aimAssistPlusToggle.checked); toast(aimAssistPlusToggle.checked?"ANCHOR AIM : ON":"ANCHOR AIM : OFF")});
  ...
  $$(".toggle-input").forEach(input=>{
    input.addEventListener("change", (e) => {
        ...
        if (item) {
            ...
            setTimeout(() => { item.style.transform = "scale(1)"; }, 120);
        }
    });
  });
...
(function(){
  ...
  function reflect(){
    ...
    document.querySelectorAll(".toggle-switch").forEach(function(sw){
      var i=sw.querySelector(".toggle-input");
      var it=sw.closest(".function-item");
      if(!i||!it) return; 
      
      var isChecked = i.checked;
      it.dataset.state = isChecked ? "on" : "off"; 
      
      if (isChecked) {
        it.style.borderColor = getComputedStyle(it).getPropertyValue('--primary') || 'rgba(34,197,94,.6)';
      } else {
        it.style.borderColor = "rgba(255,255,255,.06)";
      }
    });
  }
